...
class MRAC(object):
    def __init__(self, w0, gamma, tau_model, dt, ymprev):
        self.gamma = gamma
        self.Krprev = w0[0]
        self.Kxprev = w0[1]
    ...
    def calculate_output(self,r, ynew, update = 'lyapunov'):
        ...
        self.delta_u = self.unew - self.u_prev
        self.delta_y = ynew - self.yprev
        ...
        if update == 'lyapunov_reg':
            
            self.Kr = self.Krprev + self.dt*(-self.gamma*r*self.model_error)
            self.Kx = self.Kxprev + self.dt*(self.gamma * (ynew * self.model_error + self.delta_u * -self.delta_y * 10 ))
